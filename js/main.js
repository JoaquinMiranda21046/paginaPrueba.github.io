...
(function initLoader() {
  ...
  /* Tiempo mínimo de 2.4s para que se vea la barra completa */
  const minTime = 2400;
  ...
  if (document.readyState === 'complete') {
    ...
    // Fallback: ocultar el loader aunque las fuentes no carguen
...
(function initCursor() {
  ...
  /* Ring sigue al cursor con lag suave (lerp) */
  function lerpRing() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        observer.unobserve(entry.target); /* solo revela una vez */
      }
    });
  }, {
    threshold: 0.12,
...
(function initMockupTilt() {
  ...
  frame.addEventListener('mouseleave', () => {
    ...
    frame.style.transition = 'transform .7s var(--ease-out-expo)';
    setTimeout(() => { frame.style.transition = ''; }, 700);
  });
})();
